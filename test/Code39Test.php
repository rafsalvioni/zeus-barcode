...
class Code39Test extends \PHPUnit_Framework_TestCase
{
    ...
    public function infoTest()
    {
        ...
        $this->assertEquals($bc->getBarSet()->getBinary(), '100101101101011010010101101011001010110110110010101010100110101101101001101010100101101101');
        
        $bc = new Code39Ext('$%&b@');
        $this->assertEquals($bc->getData(), '$%&b@');
        $this->assertEquals($bc->getBarSet()->getBinary(), '10010110110101001001010010101011001011010010010100101101011001010100100101001010110110010101001010010010101101001011010100100100101001101010110100101101101');
        
        $bc = new Code39Mod43('12345', false);
        $this->assertEquals($bc->getData(), '12345F');
        $this->assertEquals($bc->getChecksum(), 'F');
        $this->assertEquals($bc->getBarSet()->getBinary(), '1001011011010110100101011010110010101101101100101010101001101011011010011010101011011001010100101101101');
        
        $bc = new Code39ExtMod43('$%&b@', false);
        $this->assertEquals($bc->getData(), '$%&b@T');
        $this->assertEquals($bc->getChecksum(), 'T');
        $this->assertEquals($bc->getBarSet()->getBinary(), '100101101101010010010100101010110010110100100101001011010110010101001001010010101101100101010010100100101011010010110101001001001010011010101101010110110010100101101101');
    }
